                    ...
                    <div class="flex-1 bg-gray-100 p-6 overflow-y-auto flex justify-center items-center">
                        @if ($latestInspection)
                            ...
                            @elseif (str_ends_with(strtolower($latestInspection->file_path), '.doc') ||
                                ...
                                <iframe {{-- src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(asset('https://abrema.gov.bi/doc/FORMULAIRE_DE_DEMANDE_INSPECTION_DES_COLIS.docx')) }}" --}}
                                    src="https://view.officeapps.live.com/op/embed.aspx?src={{ urlencode(asset(str_starts_with($latestInspection->file_path, 'doc/') ? $latestInspection->file_path : 'doc/' . $latestInspection->file_path)) }}"
                                    class="w-full h-full rounded-xl border border-gray-200" frameborder="0">
                                    <div
                                        class="w-full h-full flex flex-col items-center justify-center text-center text-gray-500">
                                        <i class="fas fa-file-word text-4xl mb-4"></i>
                                        <p>Impossible d'afficher le document Word dans ce navigateur.</p>
                                        <p class="text-sm">Le fichier est disponible dans la zone de téléversement.</p>
                                    </div>
                                </iframe>
                            @else
                ...
                @if ($latestInspection)
                    ...
                    <form action="{{ route('admin.formulaire-inspection.store') }}" method="POST"
                        ...
                        <div class="mt-4 space-y-3">
                            <p class="text-sm font-semibold text-gray-800">Remplacer le fichier</p>
                        ...
                        <div class="mt-5 text-left">
                            <label class="block text-sm font-medium text-gray-700">Nouveau titre du formulaire</label>
                            <input type="text" name="title" value="{{ old('title', $latestInspection->title) }}"
                            ...
                            @foreach ($inspections->skip(1) as $inspection)
                                <div class="flex items-center justify-between gap-3 p-3 rounded-lg bg-gray-50">
                                    <div class="min-w-0">
                                        ...
                                        <p class="text-xs text-gray-500">{{ $inspection->created_at->format('d M Y') }}
                                        </p>
                                    </div>
